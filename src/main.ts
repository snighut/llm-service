...
async function bootstrap() {
  ...
  if (workerOnly) {
    // Worker mode: No HTTP server, only process queue jobs
    await app.init();
    logger.log('🔧 Worker mode: Processing jobs from queue');
    logger.log('HTTP server NOT started');
  } else {
    ...
    logger.log(
      `🚀 API mode: Application is running on: http://localhost:${port}/api/v1`,
    );
